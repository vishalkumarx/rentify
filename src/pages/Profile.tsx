  ...
  return (
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '24px' }} className="animate-slide-in">
      ...
      <div className="glass-panel" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        <div style={{ width: '72px', height: '72px', borderRadius: '36px', background: 'var(--primary-glow)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)' }}>
          <UserIcon size={36} />
        </div>
        <div>
          <h2 style={{ fontSize: '22px', margin: '0 0 4px', fontWeight: 700 }}>Campus User</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '15px', margin: 0 }}>{user?.email || 'guest@campusrent.com'}</p>
      ...
      {/* Tab Content Mock */}
      {activeTab === 'Listings' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center', padding: '24px 0', color: 'var(--text-muted)' }}>
          {myItems.length > 0 ? (

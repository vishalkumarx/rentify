              ...
              <>
                <div className="responsive-grid" style={{ gap: '16px' }}>
                  {myItems.slice(0, 5).map(item => (
                    <div key={item.id} onClick={() => navigate(`/item/${item.id}`)} className="glass-panel" style={{ padding: '8px', display: 'flex', flexDirection: 'column', gap: '8px', cursor: 'pointer', borderRadius: '12px', border: '1px solid var(--surface-border)', position: 'relative' }}>
                      ...
                      <div style={{ position: 'absolute', top: '12px', right: '12px' }}>
                        <button 
                          ...
                          style={{ background: 'rgba(255,255,255,0.9)', border: 'none', padding: '4px', cursor: 'pointer', display: 'flex', color: '#000', borderRadius: '50%', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
                        >
                          <MoreVertical size={16} />
                        </button>
                        {openMenuId === item.id && (
                          <div style={{ position: 'absolute', top: '100%', right: '0', marginTop: '4px', background: 'var(--surface)', border: '1px solid var(--surface-border)', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 10, minWidth: '120px', overflow: 'hidden' }}>
            ...
            {myRequests.length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
                {myRequests.map((req: any) => (
                  <div key={req.id} className="glass-panel" onClick={() => { if (!req.suspended) { navigate('/community-feed', { state: { openNeedId: req.id } }); } }} style={{ background: 'var(--surface)', padding: '24px', borderRadius: '24px', display: 'flex', flexDirection: 'column', gap: '16px', border: '1px solid var(--surface-border)', position: 'relative', overflow: 'hidden', opacity: req.suspended ? 0.7 : 1, cursor: req.suspended ? 'default' : 'pointer' }}>
